				...
				<div class="mail_list">				
					...
					<div class="wbtn_div">
						...
						<c:if test="${totalPageNum >= 1}">
							...
							<a href="noticeSearch?pageNum=1" class="btn btn-outline-danger" style="margin-top:5px;"><<</a>
							<c:if test="${groupNum >1}">
								<a href="noticeSearch?pageNum=${startPageNum-1}" class="btn btn-outline-danger" style="margin-top:5px;"><</a>
							</c:if>
								<div style="display: inline-block;" class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
								 ...
								 <div class="btn-group mr-2" role="group" aria-label="First group" style="margin-top:5px;">
								 	<c:forEach begin="${startPageNum}" end="${endPageNum}" var="i">
								 		<c:if test="${pageNum==i}">
								 			<a href="noticeSearch?pageNum=${i}" class="btn btn-danger active" >${i}</a>
								 		</c:if>
								 		<c:if test="${pageNum!=i}">
								 			<a href="noticeSearch?pageNum=${i}" class="btn btn-danger" style="opacity: 0.5;">${i}</a>
								 		</c:if>
								 	</c:forEach>
								  </div>
								...
								<c:if test="${groupNum <totalGroupNum}">
									<a href="noticeSearch?pageNum=${endPageNum+1}" class="btn btn-outline-danger" style="margin-top:5px;">></a>
								</c:if>
								<a href="noticeSearch?pageNum=${totalPageNum}" class="btn btn-outline-danger" style="margin-top:5px;">>></a>
							</div>
						</c:if>
